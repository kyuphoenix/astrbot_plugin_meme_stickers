...
def draw_sticker_grid(
    ...
    with surface as canvas:
        if isinstance(background, skia.Image):
            ...
            ratio, ox, _ = get_resize_cover_ratio_and_offset(bw, bh, surface_w, surface_h)
            rw = bw * ratio
            rh = bh * ratio
            oy = 0.0
            # Top-aligned cover: crop from bottom instead of center.
            canvas.drawImageRect(
                background,
                skia.Rect.MakeXYWH(ox, oy, rw, rh),
        ...
        def draw_one(p: StickerParams):
            if debug:
                canvas.drawRect(skia.Rect.MakeWH(max_w, max_h), make_stroke_paint(0xFFFF00FF, 2))
            ...
            p = zoom_sticker(p.model_copy(deep=True), ratio)
            picture = make_sticker_picture_from_params(base_path, p, auto_resize=True, debug=debug)

            with skia.AutoCanvasRestore(canvas):
                canvas.translate(x_offset, y_offset)
                if debug:
                    canvas.drawRect(skia.Rect.MakeWH(rw, rh), make_stroke_paint(0xFFFFFF00, 2))
...
def draw_sticker_grid_from_packs(packs: list[StickerPack]):
    params: list[StickerPackCardParams] = []
    for i, p in enumerate(packs, 1):
        s = p.manifest.resolved_sample_sticker.model_copy(deep=True)
        try:
            _ = read_file_to_skia_image(p.base_path / s.base_image)
        except Exception as e:
            logger.warning(f"Skip pack `{p.slug}` in list image: {type(e).__name__}: {e}")
            continue
        params.append(
