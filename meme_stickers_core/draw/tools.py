...
def make_text_style(
    ...
    if path_fonts:
        ...
        # that do not expose TextStyle.setTypeface.
        for fp in path_fonts:
            family_name = _font_family_from_file(fp)
            if family_name:
                resolved_families.insert(0, family_name)
    if resolved_families:
        style.setFontFamilies(resolved_families)
...
def _font_family_from_file(path: str) -> str | None:
...
def read_file_to_skia_image(path: Path | str) -> skia.Image:
    ...
    data = skia.Data.MakeFromFileName(str(p))
    if data is None:
        raise ValueError(f"Failed to read image data: {p}")
