    ...
    def reload(self, clear_updating_flags: bool = False):
        ...
        for slug in slugs:
            try:
                ...
                logger.warning(f"Failed to load pack `{slug}`: {type(e).__name__}: {e}")
            else:
                op_info.succeed.append(OpIt(p))

        logger.info(f"Successfully loaded {len(self.packs)} packs")
        return op_info
        ...
        **req_kw: Unpack[ReqKwargs],
        ...
        async def do_install(info: HubStickerPackInfo):
            ...
            try:
                ...
                logger.warning(f"Failed to install pack `{info.slug}`: {type(e).__name__}: {e}")
            else:
    ...
    **req_kw: Unpack[ReqKwargs],
    ...
    async def update(p: StickerPack):
        ...
        try:
            ...
            logger.warning(f"Failed to update pack `{p.slug}`: {type(e).__name__}: {e}")
        else:
            if r:
                op_info.succeed.append(OpIt(p))
            else:
