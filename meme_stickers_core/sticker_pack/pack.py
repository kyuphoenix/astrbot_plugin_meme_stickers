...
class StickerPack:
    ...
    def reload_manifest(self, notify: bool = True):
        self.manifest = StickerPackManifest.model_validate_json(
    ...
    def reload_config(self, notify: bool = True):
        ...
        if self.config_path.exists():
            self.config = StickerPackConfig.model_validate_json(
    ...
    def merged_config(self) -> StickerPackConfig:
        ...
        if not self._cached_merged_config:
            self._cached_merged_config = StickerPackConfig(
                **deep_merge(
                    (lambda d, **kw: d.model_dump(**kw) if hasattr(d, "model_dump") else d)(self.manifest.default_config, exclude_unset=True),
                    (lambda d, **kw: d.model_dump(**kw) if hasattr(d, "model_dump") else d)(self.config, exclude_unset=True),
    ...
    def save_config(self, notify: bool = True):
        ...
        (self.base_path / CONFIG_FILENAME).write_text(
            json.dumps(self.config.model_dump(exclude_unset=True), ensure_ascii=False, indent=2),
        )
        if notify:
            self.call_callbacks()
    ...
    def save_manifest(self, notify: bool = True):
        (self.base_path / MANIFEST_FILENAME).write_text(
            json.dumps(self.manifest.model_dump(exclude_unset=True), ensure_ascii=False, indent=2),
        )
        if notify:
            self.call_callbacks()
